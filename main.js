...
function LoadTexture() {
    ...
    image.src = "https://github.com/Dmitry200285/Vgg/blob/CGW/Projection.png";
    image.onload = () => {
        gl.bindTexture(gl.TEXTURE_2D, texture);
        gl.texImage2D(
            gl.TEXTURE_2D,
...
window.onkeydown = (e) => {
    ...
    if (e.keyCode == 87) { //w
        pointPos[0] = Math.min(pointPos[0] + v, Math.PI * 2);
    }
    else if (e.keyCode == 65) { //a
        pointPos[1] = Math.max(pointPos[1] - v, -2.5);
    }
    else if (e.keyCode == 83) { //s
        pointPos[0] = Math.max(pointPos[0] - v, 0);
    }
    else if (e.keyCode == 68) { //d
        pointPos[1] = Math.min(pointPos[1] + v, 2.5);
    }
    draw();
}